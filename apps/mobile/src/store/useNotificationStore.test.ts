...
import { useNotificationStore, type NotificationBase } from './useNotificationStore';

let nextId = 0;
const factory = (overrides: Partial<NotificationBase> = {}): NotificationBase => ({
  id: `notif-${nextId++}`,
  ticketId: 'T-100',
  actor: 'Agent',
...
describe('useNotificationStore', () => {
  ...
  it('marks all notifications for a ticket as read', () => {
    ...
    const state = useNotificationStore.getState();
    expect(state.notifications.filter((n) => n.ticketId === 'ticket-a').every((n) => n.read)).toBe(true);
    expect(state.notifications.find((n) => n.ticketId === 'ticket-b')?.read).toBe(false);
    expect(state.unreadCount).toBe(1);
  });
