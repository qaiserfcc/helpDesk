...
import { login, refreshSession } from './authService.js';
...
describe('login', () => {
  it('returns user details and tokens for valid credentials', async () => {
    ...
    const result = await login({ email: baseUser.email, password: 'ValidPass123!' });
  ...
  it('throws when credentials do not match', async () => {
    ...
    await expect(login({ email: baseUser.email, password: 'WrongPass789!' })).rejects.toMatchObject({
      status: 401
    });
  });
});
...
describe('refreshSession', () => {
  it('issues a new token pair for a valid refresh token', async () => {
    ...
    const { refreshToken } = createTokenPair({ id: baseUser.id, role: baseUser.role });
  ...
  it('rejects invalid refresh tokens', async () => {
    await expect(refreshSession('not-a-real-token')).rejects.toMatchObject({ status: 401 });
  });
});
