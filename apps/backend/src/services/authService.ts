...
export async function login({ email, password }: LoginInput): Promise<AuthResult> {
  const userRecord = await prisma.user.findUnique({ where: { email }, select: loginSelect });
...
export async function refreshSession(refreshToken: string): Promise<AuthResult> {
  ...
  const userRecord = await prisma.user.findUnique({ where: { id: payload.sub }, select: safeUserSelect });
