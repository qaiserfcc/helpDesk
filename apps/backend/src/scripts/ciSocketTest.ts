...
async function main() {
  ...
  const email =
    process.env.EMAIL || process.env.ADMIN_EMAIL || "admin@helpdesk.local";
  const password =
    process.env.PASSWORD || process.env.ADMIN_PASSWORD || "ChangeMe123!";
  ...
  try {
    ...
    if (status === 401 || status === 400) {
      ...
      const tempEmail =
        ...
        `test+ci-${Math.random().toString(36).slice(2)}@helpdesk.local`;
      const tempPassword =
        process.env.TEST_PASSWORD ||
        `P@ss${Math.random().toString(36).slice(2)}!`;
      try {
        const reg = await axios.post(
          `${baseUrl}/auth/register`,
          { name: "CI Test", email: tempEmail, password: tempPassword },
          { timeout: 10000 },
        );
        ...
        console.error(
          "Register failed",
          regErr?.response?.data || regErr?.message || regErr,
        );
        process.exit(4);
      }
    }
  ...
  socket.on("tickets:created", (payload: any) => {
    try {
      ...
      if (expectedTicketId && id !== expectedTicketId) {
        console.warn("tickets:created for unrelated ticket ignored", {
          expectedTicketId,
          received: id,
        });
        return;
